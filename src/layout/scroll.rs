...
impl<M: Clone + 'static> ScrollView<M> {
    ...
    /// in that margin - where the eye expects it, and where a themed list puts
    /// its own.
...
///
/// Large enough that nothing real reaches it, small enough that a stack can
/// add several of them without leaving `i32`.
pub const UNBOUNDED: i32 = 1 << 24;
