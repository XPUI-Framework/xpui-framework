    ...
    fn measure(&mut self, available: Size) {
        ...
        for child in &mut self.children {
            if child.is_flexible() {
                ...
                // Saturating: a child offered an unbounded height may report
                // one, and three of those must not wrap into a negative.
                fixed_main = fixed_main.saturating_add(axis.main(child.size()));
            }
        }
